...
const getColumnHeaderStyle = (index:number) =>{
    ...
    return index % 2 == 0 ? {
        ...
        backgroundColor: 'green', // Example: Set the text color to red
        fontWeight: 'bold', // Example: Set the font weight to bold
        display:'flex',
        width:'100%',
        height:'100px',
        ...
        <el-row :gutter="20">
            <el-col :span="24" :xs="24">
                ...
                <el-card>
                    <el-table  :data="signin_bonous_list" style="width: 100%;">
                        ...
                        <el-table-column v-for="(item, index) in signin_bonous_list[0].days" :key="index" :label="`第${index+1}天签到`" align="center" class="headerColor">
                            <template #header>
                                <span :style="getColumnHeaderStyle(index)">
                                    {{`第${index+1}天签到`}}
                                </span>
                            </template>
                            <el-table-column label="金额" align="center" :prop="`days[${index}].amount`">
                                <template #header>
                                    <span :style="getColumnHeaderStyle(index)">
                                        {{`金额`}}
                                    </span>
                            ...
                            <el-table-column label="打码倍率" align="center" :prop="`days[${index}].code_magnification`">
                                <template #header>
                                    <span :style="getColumnHeaderStyle(index)">
                                        {{`打码倍率`}}
                                    </span>
                            ...
                            <el-table-column label="完成签到需求充值量" align="center" prop="requried_charge_amount">
                                <template #header>
                                    <span :style="getColumnHeaderStyle(index)">
                                        {{`完成签到需求充值量`}}
                                    </span>
                            ...
                            <el-table-column label="完成签到需求打码量" align="center" prop="required_code_amount">
                                <template #header>
                                    <span :style="getColumnHeaderStyle(index)">
                                        {{`完成签到需求打码量`}}
                                    </span>
            ...
            <el-dialog v-model="showDetailDialog" :title="dialog_title">
                ...
                <div class="dl_row" v-for="(item, index) in signin_bonous_item.days" :key="index">
                    ...
                    <p v-else style="width:30%">
                        第8天及之后奖励
                    </p>
                    <p style="width:20%">
                        奖励金额
                    </p>
                    <el-input style="width:25%" v-model="signin_bonous_item.days[index].amount" placeholder="请输入奖励金额"/>
                    <p style="width:20%; margin-left:50px">
                        打码倍率
                    </p>
                    <el-input style="width:25%" v-model="signin_bonous_item.days[index].code_magnification" placeholder="请输入打码l倍率"/>
                </div>
                <div class="dl_row">
                    <p style="width:30%">
                        完成签到需要的充值金额
                    </p>
                    <el-input v-model="signin_bonous_item.requried_charge_amount" placeholder="请输入完成签到需要的充值金额"/>
                </div>
                <div class="dl_row">
                    <p style="width:30%">
                        完成签到需要的打码量
                    </p>
                    <el-input v-model="signin_bonous_item.required_code_amount" placeholder="请输入完成签到需要的打码量"/>
                </div>
                <template #footer>
